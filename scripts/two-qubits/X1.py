...
def run(n_training_iterations=1, save=True, plot=True):
    ...
    try:
        ...
        if save is True:
            ...
            sr = SaveResults(env, alg, results=list_of_results, save_path = RESULTS_DIR + "two-qubit gates/"+"X1_" + datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S/"))
