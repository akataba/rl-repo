...
def run(n_training_iterations=1, save=True, plot=True):
    ...
    try:
        ...
        # ------------------------------------------------------------------------
        path_to_detuning = "C:\\Users\\haknamu\\Documents\\PythonWorkSpace\\GateSynth\\rl-repo\\src\\relaqs\\quantum_noise_data\\qubit_detuning_data.json"
        ...
        if save is True:
            ...
            sr = SaveResults(env, alg, results=list_of_results, save_path = RESULTS_DIR + "QCE2023/"+"05_noisy_detuning_H__" + datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S/"))
