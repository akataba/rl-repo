...
from relaqs.quantum_noise_data.get_data import get_month_of_single_qubit_data, get_month_of_all_qubit_data, get_single_qubit_detuning
from relaqs import quantum_noise_data
from relaqs import QUANTUM_NOISE_DATA_DIR
from relaqs import RESULTS_DIR
from relaqs import get_results_dir
import os
...
def run(n_training_iterations=1, save=True, plot=True):
    ...
    try:
        ...
        path_to_file = os.path.join(QUANTUM_NOISE_DATA_DIR, "april", "ibmq_belem_month_is_4.json")
        t1_list, t2_list = get_month_of_all_qubit_data(path_to_file)        #in seconds
        # ------------------------------------------------------------------------
        path_to_detuning = os.path.join(QUANTUM_NOISE_DATA_DIR, "qubit_detuning_data.json")
        detunings = get_single_qubit_detuning(path_to_detuning)
        ...
        if save is True:
            ...
            sr = SaveResults(env, alg, results=list_of_results, save_path = os.path.join(get_results_dir(),"QCE2023", "05_noisy_detuning_H__" + datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S/")))
